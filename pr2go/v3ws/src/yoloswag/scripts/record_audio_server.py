...
def handle_record_audio(req):
  ...
  (hypothesis, confidence) = dothis(req.d, req.cmds, filename_base)
    
  return RecordAudioResponse(hypothesis, confidence)	

def record_audio_server():
  rospy.init_node('record_audio_server', log_level=rospy.INFO)
